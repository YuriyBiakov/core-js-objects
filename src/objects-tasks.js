...
function mergeObjects(objects) {
  ...
  objects.forEach((object) => {
    ...
    objectKeys.forEach((keyName) => {
      ...
      } else result[keyName] = object[keyName];
    });
  });
  // 'entries'
  return result;
...
function compareObjects(obj1, obj2) {
  ...
  let result = false;
  if (obj1Keys.length !== Object.keys(obj2).length) return result;
  result = true;
  obj1Keys.forEach((element) => {
    if (obj1[element] !== obj2[element]) result = false;
  });
  return result;
}
...
function group(array, keySelector, valueSelector) {
  ...
  function addElement(object, map, index) {
    const mapKey = keySelector(array[index]);
    const mapVal = valueSelector(array[index]);
    if (map.has(mapKey)) {
      const arr = map.get(mapKey);
      arr.push(mapVal);
    } else map.set(mapKey, new Array(mapVal));
  }
  for (let i = 0; i < array.length; i += 1) {
    const obj = array[i];
    addElement(obj, resultMap, i);
  }
  return resultMap;
}
